var T = {
  ...
  levelUp: "<div class='tweet' style='background:green;color: white;'>BRAVO PATRIOTE! TU PASSES AU NIVEAU D'ACCREDIATION SUPERIEUR: {{level}}</div>",
  popup: "<div class='popup'><a href='{{urlProfile}}' target='_blank'><img src='{{img}}' alt='mugshot' /></a> <span>{{name}}</span> : </a>  <a href='{{urlTweet}}' target='_blank'>{{txt}}</a><div style='clear: both;'></div> <hr/> <button class='closePopupBtn' style='float:right'>Fermer</button> <button class='reportPopupBtn'>Dénoncer</button></div>",
  getId: "Veuillez decliner votre identité:",
  noAnon: "L'anonymat est intedit!",
